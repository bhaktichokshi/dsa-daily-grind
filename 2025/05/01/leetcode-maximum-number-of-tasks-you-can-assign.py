# Problem: 2071. Maximum Number of Tasks You Can Assign [https://leetcode.com/problems/maximum-number-of-tasks-you-can-assign/description/?envType=daily-question&envId=2025-05-01]
# Difficulty: Hard

from collections import deque
from typing import List

# My solution: O(nlogn), O(1)
# 1. Sort the tasks and workers in ascending order. 
# 2. Use binary search to find the maximum number of tasks that can be assigned.
# 3. Define a helper function 'check' to determine if 'x' tasks can be assigned to workers.
# 4. In the 'check' function, iterate through the workers and tasks:
#    - Use a deque to keep track of tasks that can be assigned to the current worker.
#    - If the current worker can do the task without a pill, assign it.
#    - If the current worker cannot do the task without a pill, check if pills are available.
#    - If pills are available, assign the task and decrement the pill count.
# 5. If all tasks can be assigned, return True; otherwise, return False.
# 6. Perform a binary search to find the maximum number of tasks that can be assigned.
# 7. Return the maximum number of tasks that can be assigned.
# 8. The 'left' variable now holds the maximum number of tasks that can be assigned.
# 9. Return 'left' as the final result.
class Solution:
